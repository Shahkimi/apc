...
<div class="container-fluid  ">
	...
	<form enctype="multipart/form-data" class="bg-light rounded p-4 shadow-lg" action="{{url('/apc/edit')}}" method="POST" style="width: 1000px; margin: auto;">
		...
		<div class="row">
			...
			<div class="col-lg-3 form-group">
				...
				<select name="sesi" id = 'sesi' class="custom-select" required>
		            ...
		            <option value="PAGI_LEWAT" {{($pegawai->sesi=="PAGI_LEWAT")? "selected" : "" }}>PAGI (LEWAT)</option>
		            <option value="PETANG_LEWAT" {{($pegawai->sesi=="PETANG_LEWAT")? "selected" : "" }}>PETANG (LEWAT)</option>
		        </select>
			</div>
			...
			<div class="col-lg-3 form-group">
				...
				<select name="kehadiran" class="custom-select" id="kehadiran" required>
		            ...
		            <option value="0" {{($pegawai->kehadiran=="0")? "selected" : "" }}>TIDAK/BELUM MENDAFTAR</option>
		        </select>
	        </div>
		...
		if (sesi.value=='PAGI_LEWAT'||sesi.value=='PETANG_LEWAT') {
			axios.get('/apc/public/apc/getNoLewat/'+sesi.value)
	                .then(function (response) {
						...
						$("#no_terakhir_lewat").text("No "+sesi.value+" terakhir = "+response.data);
	                })
