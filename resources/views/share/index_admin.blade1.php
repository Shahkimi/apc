...
<br><br>
    ...
    <table class="table">
        <thead>
            <tr>    
                <th scope="col">
                    <a href="{{ route('share.create') }}" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Tambah</a>
                </th>
                <th colspan="4"><h2>SENARAI</h2></th>   
            </tr>
        </thead>
    </table>
                ...
                <th scope="col">
                    <select class="form-control btn-sm" name="jenis_id" form="search">
                        ...
                        @foreach ($jenis as $tjenis)
                            <option value="{{$tjenis->id}}" {{($tjenis->id == $fjenis)? "selected" : "" }}>{{$tjenis->jenis}}</option>
                        @endforeach
                    </select>
                </th>
